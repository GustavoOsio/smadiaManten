...
@section('content')
    ...
    <form id="posts" action="{{ route('budget.update',$budget->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row justify-content-center">
            <div class="col-xs-6 col-sm-4 col-md- col-lg-2">
                <div class="form-group">
                    ...
                    <select name="mouth" id="mouth" class="form-control" required>
                        ...
                        @for($i=01;$i<=12;$i++)
                            <option value="{{$i}}" {{($i == $budget->mouth) ? 'selected':''}}>{{$meses[$i]}}</option>
                        @endfor
                    </select>
                </div>
            </div>
            ...
            <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2">
                <div class="form-group">
                    ...
                    <input type="number" name="value" class="form-control" placeholder="Valor" required value="{{ $budget->value }}">
                </div>
            </div>
            <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2">
                <div class="form-group">
                    {{--<strong>Apellidos: <span>*</span></strong>--}}
                    <input type="number" name="patients" class="form-control" placeholder="Cantidad de pacientes" required value="{{ $budget->patients }}">
                </div>
            </div>
        </div>
        <div class="separator"></div>
