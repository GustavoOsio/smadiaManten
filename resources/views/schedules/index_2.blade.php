...
@section('content')
    ...
    <table id="table-soft-schedules_3" class="table-patients table-striped">
        ...
        <tbody>
            <tr class="search-table-input">
                <th>
                    <input id="id_shedule" value="{{$request->id}}">
                </th>
                <th>
                    <input id="patient_shedule" value="{{$request->patient}}">
                </th>
                <th>
                    <input id="cc_shedule" value="{{$request->cc}}">
                </th>
                <th>
                    <input id="phone_shedule" value="{{$request->phone}}">
                </th>
                <th>
                    <input id="contract_shedule" value="{{$request->contract}}">
                </th>
                <th>
                    <input id="professional_shedule" value="{{$request->professional}}">
                </th>
                <th>
                    <select id="service_shedule">
                ...
                <th>
                    <input id="date_shedule" value="{{$request->date}}">
                </th>
                <th>
                    <input id="hour_shedule" value="{{$request->hour}}">
                </th>
                <th>
                    <input id="hourend_schedule" value="{{$request->hourend}}">
                </th>
                <th>
                    <select id="status_schedule_go">
                        <option value="">Seleccionar</option>
                        ...
                        <option value="fallida" {{$request->status=='fallida'?'selected':''}}>Fallida</option>
                ...
                <th>
                    <select id="confirm_schedule">
                        <option value="">Seleccionar</option>
                        <option value="si" {{$request->confirm=='si'?'selected':''}}>SI</option>
                        <option value="no" {{$request->confirm=='no'?'selected':''}}>NO</option>
                    </select>
                </th>
                <th>
                    <input id="update_schedule" value="{{$request->update}}">
                </th>
                <th>
                    <input id="comment_schedule" value="{{$request->comment}}">
                </th>
                <th>
                    <input id="observations_schedule" value="{{$request->observations}}">
                </th>
                <th></th>
                <th>
                    <input id="create_date_schedule" value="{{$request->createdate}}">
                </th>
            </tr>
        @php
            $sum_update = 0;
        ...
        @foreach ($schedules as $schedule)
            @php
                $validate = true;
                $name = $schedule->user->name.' '.$schedule->user->lastname;
                //echo $name;
                if($request->update != ''){
                    if(stripos($name,$request->update) > 0 || stripos($name,$request->update) === 0){
                        $validate = true;
                        $sum_update = $sum_update + 1;
                    }else{
                        $validate = true;
                    }
                }
            @endphp
            @if($validate == true)
            <tr class="tr_click" id="{{ $schedule->patient_id }}">
                <td>{{ $schedule->id }}</td>
                ...
                <td style="text-align: center">{{ $schedule->confirm }}</td>
                ...
                @if ($schedule->status == "programada" || $schedule->status == "confirmada" || $schedule->status == "en sala")
                    ...
                    @if ($schedule->status == "atendida")
                        ...
                        @endcan
                    @endif
                    @if ($schedule->status == "programada" && $schedule->confirm == 'no')
                        @can('update', \App\Models\Schedule::class)
                ...
                <td>
                    {{ date("Y-m-d", strtotime($schedule->created_at)) }}
                </td>
            </tr>
            @endif
        @endforeach
        ...
        @php
            if($request->update != ''){
                $total_sche = $sum_update;
            }else{
                $total_sche = $schedules->total();
            }
        @endphp
        <tr>
            <th></th>
            <th></th>
            <th></th>
            <th>Total: <span class="total">{{$schedules->total()}}</span></th>
            <th></th>
            <th></th>
            <th></th>
            <th></th>
    ...
    @component('schedules.reservation',[
        'request'=>$request,
...
@section('script')
    <script>
        $(document).ready(function() {
            $('#date_shedule,#create_date_schedule').datepicker({
                locale: 'es',
                dateFormat:"yy-mm-dd",
                closeText: 'Cerrar',
                prevText: '<Ant',
            ...
            let $id_shedule,$patient_shedule,$status_schedule,$cc_shedule,$phone_shedule,$contract_shedule,$professional_shedule,$service_shedule,$date_shedule;
            let $hour_shedule,$hourend_schedule,$update_schedule,$create_date_schedule,$comment_schedule,$observations_schedule,$confirm_schedule;
            $(function () {
                $id_shedule = $('#id_shedule');
                $patient_shedule = $('#patient_shedule');
                $cc_shedule = $('#cc_shedule');
                $phone_shedule = $('#phone_shedule');
                $contract_shedule = $('#contract_shedule');
                $professional_shedule = $('#professional_shedule');
                $service_shedule = $('#service_shedule');
                $date_shedule = $('#date_shedule');
                $hour_shedule = $('#hour_shedule');
                $hourend_schedule = $('#hourend_schedule');
                $status_schedule = $('#status_schedule_go');
                $update_schedule = $('#update_schedule');
                $comment_schedule = $('#comment_schedule');
                $observations_schedule = $('#observations_schedule');
                $create_date_schedule = $('#create_date_schedule');
                $confirm_schedule = $('#confirm_schedule');

                $status_schedule.change(onChangeFilter);
                $service_shedule.change(onChangeFilter);
                $confirm_schedule.change(onChangeFilter);
            });
            $('#id_shedule,#patient_shedule,#cc_shedule,#phone_shedule,#contract_shedule,#professional_shedule').keypress(function (e) {
                if(e.which == 13) {
                    onChangeFilter();
                }
            });
            $('#date_shedule,#hour_shedule,#hourend_schedule,#update_schedule,#comment_schedule,#observations_schedule,#create_date_schedule').keypress(function (e) {
                if(e.which == 13) {
                    onChangeFilter();
                }
            });
            ...
            function onChangeFilter(){
                location.href = '/schedules?id='+$id_shedule.val()+
                    '&patient='+$patient_shedule.val()+
                    '&cc='+$cc_shedule.val()+
                    '&phone='+$phone_shedule.val()+
                    '&contract='+$contract_shedule.val()+
                    '&professional='+$professional_shedule.val()+
                    '&service='+$service_shedule.val()+
                    '&date='+$date_shedule.val()+
                    '&hour='+$hour_shedule.val()+
                    '&hourend='+$hourend_schedule.val()+
                    '&status='+$status_schedule.val()+
                    '&update='+$update_schedule.val()+
                    '&comment='+$comment_schedule.val()+
                    '&observations='+$observations_schedule.val()+
                    '&createdate='+$create_date_schedule.val()+
                    '&confirm='+$confirm_schedule.val();
            }
        });
    </script>
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#table-soft-schedules_3 tbody').on('dblclick', '.tr_click', function () {
