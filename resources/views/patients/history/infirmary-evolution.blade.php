        ...
        <tr idtype="{{$mH->id_type}}" idrelation="{{$mH->id_relation}}" patientid="{{$mH->patient_id}}">
            <td style="text-align: center;">
                <div class="form-check">
                    <input class="form-check-input printhc" data-tipo="infirmary-evolution" data-id="{{$mH->id_relation}}" type="checkbox" value="{{$mH->id_relation}}">
                </div>
            </td>
            <td style="text-align: center;">{{date("Y-m-d", strtotime($mH->date))}}</td>
            <td style="text-align: center;">{{$mH->NameUser}} {{$mH->LastNameUser}}</td>
            @php
            $value = \App\Models\InfirmaryEvolution::where('id',$mH->id_relation)->first();
            @endphp
            <td style="text-align: center;">
                {{$value->evolution}}
            </td>
