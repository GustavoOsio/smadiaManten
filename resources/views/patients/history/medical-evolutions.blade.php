        ...
        <tr idtype="{{$mH->id_type}}" idrelation="{{$mH->id_relation}}" patientid="{{$mH->patient_id}}">
            <td style="text-align: center;">
                <div class="form-check">
                    <input class="form-check-input printhc" data-tipo="medical-evolutions" data-id="{{$mH->id_relation}}" type="checkbox" value="{{$mH->id_relation}}">
                </div>
            </td>
            @php
            $porciones = explode("/", $mH->date);
            @endphp
            <td style="text-align: center;">{{$porciones[2].'-'.$porciones[1].'-'.$porciones[0]}}</td>
            <td style="text-align: center;">{{$mH->NameUser}} {{$mH->LastNameUser}}</td>
            @php
            $value = \App\Models\MedicalEvolutions::where('id',$mH->id_relation)->first();
            @endphp
            <td style="text-align: center;">
                {{$value->observations}}
            </td>
