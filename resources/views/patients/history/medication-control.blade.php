        ...
        <tbody>
            @foreach($medicalHistory as $key => $mH)
                ...
                <div class="form-check">
                    <input class="form-check-input printhc" data-tipo="medication-control" data-id="{{$mH->id_relation}}" type="checkbox"  value="{{$mH->id_relation}}">
                </div>
            </td>
                        <td style="text-align: center;" >{{date("Y-m-d", strtotime($mH->date))}}</td>
                        <td style="text-align: center;" >{{$mH->NameUser}} {{$mH->LastNameUser}}</td>
                        @php
                            $value = \App\Models\MedicationControl::where('id',$mH->id_relation)->first();
                        @endphp
                        <td style="text-align: center;" >
                            {{$value->service}}
                        </td>
