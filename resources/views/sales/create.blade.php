                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="position: relative;">
                                ...
                                <input type="text" class="form-control" name="identy" id="patient_identy"
                                       onkeypress="return soloNumeros(event);"
                                       onkeyup="searchPatient(this.value)" required autocomplete="off">
                                <div id="resultPatients"></div>
                            </td>
                            <td style="position: relative">
                                <input type="text" class="form-control" id="patient_name" onkeyup="searchPatientName(this.value)" autocomplete="off">
                                <div id="resultPatients_2"></div>
                            </td>
                            <td><input type="text" class="form-control" readonly id="patient_city"></td>
                            <td><input type="text" class="form-control" readonly id="patient_address"></td>
                            <td><input type="text" class="form-control" readonly id="patient_cellphone"></td>
                        </tr>
                        @endif
                    </tbody>
                </table>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="text" name="amount" id="amount" readonly class="form-control" placeholder="0" required>
                            </div>
                        </div>
                    </div>
                    <!--
                ...
                <div class="row" style="margin-bottom: 0px">
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <select name="partner_discount" class="form-control" required>
                            ...
                            <div class="row" style="margin-bottom: 0px">
                                ...
                                <div class="col-md-6 priceOne">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control OnlyNumber" onkeyup="formatNumberWrite(this)" name="amount_one" id="amount_one">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12 online">
                            <div class="form-group" style="line-height: 24px">
                                <div class="row mt-2">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <strong>Numero de aprobacion</strong>
                        ...
                        <div class="col-md-12">
                            <div class="row" style="margin-bottom: 0px">
                                ...
                                <div class="col-md-6 priceTwo">
                                    <div class="form-group">
                                        <label class="c-primary font-weight-bold">Valor</label>
                                        <input type="text" class="form-control OnlyNumber" onkeyup="formatNumberWrite(this)" name="amount_two" id="amount_two">
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        function calcule(pos) {
            ...
            if($("#desc-" + pos).val() == '' || $("#desc-" + pos).val() == 0){
                desc_cop[pos] = $("#desc_cop-" + pos).val().replace(".", "");
                desc_cop[pos] = desc_cop[pos].replace(".", "");
                desc_cop[pos] = desc_cop[pos].replace(",", "");
                desc_cop[pos] = desc_cop[pos].replace(",", "");
            }else{
                desc[pos] = $("#desc-" + pos).val().replace(".", "");
                desc[pos] = desc[pos].replace(".", "");
                desc[pos] = desc[pos].replace(",", "");
                desc[pos] = desc[pos].replace(",", "");
            }
            ...
            if($("#desc-" + pos).val() == '' || $("#desc-" + pos).val() == 0){
                total[pos] = (price[pos] * qty[pos] ) - desc_cop[pos];
                if(desc[pos] >= (price[pos] * qty[pos])){
                    total[pos] = 0;
                    desc[pos] = price[pos] * qty[pos];
                    $("#desc_cop-" + pos).val(price[pos] * qty[pos]);
                }
            }else{
                total[pos] = (price[pos] - ( price[pos]*desc[pos]/100 ) ) * qty[pos];
                if(desc[pos] >= 100){
                    total[pos] = 0;
                    desc[pos] = 100;
                    $("#desc-" + pos).val(100);
        ...
        function calculeTax() {
            var tax_total = 0,totaly_price = 0; sub_amount = 0;
            var i = 1;
            tax.forEach(function (t) {
                if (t > 0)

                    if(parseInt(desc[i]) === 0 || desc[i] === undefined){
                      totaly_price = parseInt(price[i]) - parseInt(desc_cop[i]);
                    }else{
                      totaly_price = parseInt(price[i]) - (parseInt(price[i]) * (parseInt(desc[i]) / 100));
                    }

                    tax_total += (totaly_price * parseInt(qty[i])) * parseInt(t) / 100;
                    sub_amount += price[i] * qty[i];
                    console.log(totaly_price+' '+tax_total);
                i++;
            });
            $("[name=tax]").val(tax_total);
            $("[name=sub_amount]").val(formatNumber(sub_amount));
        }

        function amountT() {
            amount = 0; sub_amount = 0;
            for (var i = 1; i < total.length; i++) {
                amount += parseInt(total[i]);
                sub_amount += parseInt(price[i] * qty[i]);
            }
            $("[name=sub_amount]").val(formatNumber(sub_amount));
        ...
        function descT() {
            ...
            for (var i = 1; i < desc.length; i++) {
                if($("#desc-" + i).val() == '' || $("#desc-" + i).val() == 0){
                    desc_totaly += (parseInt(desc[i]));
                }else{
                    desc_totaly += (parseInt(price[i]) * parseInt(desc[i]) / 100) * qty[i];
                }
            }
            $("[name=discount_total]").val(formatNumber(desc_totaly));
        }
        ...
        function cloneRowSale(obj) {
            ...
            $("#tableToModify").append('<tr id="rowToClone-' + countS +
                ...
                '" name="tax_p[]" readonly data-id="' + countS + '" class="form-control"></td>' +
                '<td width="100px"><input type="text" min="0" max="100" maxlength="3" id="desc-' + countS +
                '" name="desc[]" onkeypress="return soloNumeros(event);" onkeyup="calcule_des(this,'+ countS +')" data-id="' +
                countS + '" class="form-control" value="0" required></td>' +
                '<td width="100px"><input type="text" min="0" max="100" maxlength="100" id="desc_cop-' + countS +
                '" name="desc_cop[]" onkeypress="return soloNumeros(event);" onkeyup="calcule_des(this,'+ countS +');" data-id="' +
                countS + '" class="form-control" value="0" required></td>' +
